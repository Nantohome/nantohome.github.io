...
const validation = {
  isNotEmpty: (e) => {
    ...
    if(typeof(CKEDITOR) !== 'undefined' && CKEDITOR.instances[e.name] !== undefined) {
...
window.onload = () => {
  ...
  const toggleHeader = (hidden) => {
    ...
    hidden && header.classList[((currentScroll < lastScroll) || (currentScroll <= 10)) ? 'remove' : 'add']('hidden')
    ...
    switch(type) {
      case 'main':
        window.swipers.main = new Swiper(swiperList[i], {
          ...
          on: {
            init: (s) => {
              const videoBtns = Array.from(s.el.querySelectorAll('[data-video-btn]'))
              videoBtns.map(btn => btn.addEventListener('click', ({ target }) => {
                target = getParentByNode(target, 'BUTTON')
                target.classList.toggle('active')
                if(target.classList.contains('active')) {
                  s.el.classList.toggle('active')
                  s.el.classList.add('animate1')
                  setTimeout(() => s.el.classList.add('animate2'), 150)
                } else {
                  s.el.classList.remove('animate2')
                  setTimeout(() => {
                    s.el.classList.remove('animate1')
                    setTimeout(() => s.el.classList.toggle('active'), 150)
                  }, 150)
                }
                setTimeout(() => s.el.nextElementSibling.classList.toggle('opacity'), 150)
                document.body.classList.toggle('overflow')
                if(document.body.classList.contains('overflow')) {
                  swiperList[i].parentNode.parentNode.classList.add('fullscreen')
                } else {
                  setTimeout(() => swiperList[i].parentNode.parentNode.classList.remove('fullscreen'), 700)
                }
              }))
            }
          }
      ...
      case 'main_thumb':
        new Swiper(swiperList[i], {
          ...
          allowTouchMove: false,
          autoHeight: true,
          navigation: { nextEl: '.main_thumb_next', prevEl: '.main_thumb_prev' },
          on: {
            slideChange: (s) => window.swipers.main.slideTo(s.realIndex)
          }
        }) 
        break
      case 'main_side':
      ...
      case 'news':
        ...
        break
      case 'quiz':
        const stepbox = swiperList[i].querySelector('[data-quiz-steps]')
        const setQuizSteps = (s) => {
          if(stepbox) stepbox.innerHTML = `Шаг ${s.realIndex + 1}<span>из ${s.slides.length}</span>`
          s.el.classList[((s.realIndex + 1) === s.slides.length) ? 'add' : 'remove']('hidden')
        }
        const quizSteps = Array.from(document.querySelectorAll('[data-quiz-step]'))
        const quizPrev = document.querySelector('[data-quiz-step="prev"]')
        ...
        for(let i in quizSteps) {
          quizSteps[i].addEventListener('click', ({ target }) => {
            ...
            const validations = Array.from(slide.querySelectorAll('[data-validate]'))
            let err = false
            for(let e in validations) {
              ...
              switch(rule) {
                case 'text':
                  ...
                  validations[e].classList[!check ? 'add' : 'remove']('error')
                  if(!check) err = true
                  break
                case 'number':
            ...
            if(!err) {
              if(target.getAttribute('data-quiz-step') === 'prev') {
                window.swipers.quiz.slidePrev()
              } else {
                window.swipers.quiz.slideNext()
              }
              quizPrev.classList[idx ? 'remove' : 'add']('disable')
            }
          })
        }
        break
      case 'history':
        window.swipers.history = new Swiper(swiperList[i], {
          slidesPerView: 4,
          spaceBetween: 16
        }) 
        const historyStepBtns = Array.from(swiperList[i].querySelectorAll('[data-history-step]'))
        for(let i in historyStepBtns) {
          historyStepBtns[i].addEventListener('click', ({ target }) => {
            if(!target.classList.contains('active')) {
              const current = historyStepBtns.find(btn => btn.classList.contains('active'))
              current && current.classList.remove('active')
              target.classList.add('active')
              const step = target.getAttribute('data-history-step')
              const index = window.swipers.history.slides.findIndex(slide => slide.getAttribute('data-history-target') == step)
      ...
      case 'values':
        new Swiper(swiperList[i], {
          slidesPerView: 4,
          spaceBetween: 16,
          navigation: { nextEl: '.values_next', prevEl: '.values_prev' }
        }) 
        break
      case 'career':
        new Swiper(swiperList[i], {
          ...
          fadeEffect: { crossFade: true },
          autoHeight: true,
          navigation: { nextEl: '.next_carrer', prevEl: '.prev_carrer' }
        }) 
        break
      case 'contacts':
        new Swiper(swiperList[i], {
          ...
          fadeEffect: { crossFade: true },
          autoHeight: true,
          navigation: { nextEl: '.next_contacts', prevEl: '.prev_contacts' }
        }) 
        break
      case 'where_to_buy':
        window.swipers.whereToBuy = new Swiper(swiperList[i], {
          ...
          fadeEffect: { crossFade: true },
          autoHeight: true,
          navigation: { nextEl: '.next_where_to_buy', prevEl: '.prev_where_to_buy' }
        }) 
        break
      case 'contract_production':
        ...
        const contractProductionNavs = Array.from(document.querySelectorAll('[data-contract-production-step]'))
        for(let i in contractProductionNavs) {
          contractProductionNavs[i].addEventListener('click', ({ target }) => {
            target = getParentByNode(target, 'BUTTON')
            if(!target.classList.contains('active')) {
              const active = document.querySelector('[data-contract-production-step].active')
              active && active.classList.remove('active')
              target.classList.add('active')
              window.swipers.contractProduction.slideTo(parseInt(target.getAttribute('data-contract-production-step')))
            }
          })
        }
      ...
      case 'contract_production_why':
        new Swiper(swiperList[i], {
          slidesPerView: 4,
          spaceBetween: 16,
          navigation: { nextEl: '.contract_production_why_next', prevEl: '.contract_production_why_prev' }
        }) 
        break
      case 'gallery':
      ...
      case 'gallery_thumb':
        new Swiper(swiperList[i], {
          ...
          slidesPerView: 4,
          spaceBetween: 16,
          navigation: { nextEl: '.gallery_thumb_next', prevEl: '.gallery_thumb_prev' },
          breakpoints: {
            1024: {
              direction: 'vertical'
            }
          ...
          on: {
            init: (s) => {
              for(let i in s.slides) {
                ...
                s.slides[i].addEventListener('click', ({ target }) => {
                  if(!s.slides[i].classList.contains('active')) {
                    window.swipers.gallery.slideTo(i)
                    s.slideTo(i)
                    const active = s.slides.find(slide => slide.classList.contains('active'))
                    active && active.classList.remove('active')
                    target = getParentByClassname(target, 'swiper-slide')
                    target.classList.add('active')
                  }
                })
              }
            },
            slideChange: (s) => window.swipers.gallery.slideTo(s.realIndex)
          }
        }) 
        break
      case 'product':
        ...
        const productParamsNavs = Array.from(document.querySelectorAll('[data-product-step]'))
        for(let i in productParamsNavs) {
          productParamsNavs[i].addEventListener('click', ({ target }) => {
            target = getParentByNode(target, 'BUTTON')
            if(!target.classList.contains('active')) {
              const active = document.querySelector('[data-product-step].active')
              active && active.classList.remove('active')
              target.classList.add('active')
              window.swipers.contractProduction.slideTo(parseInt(target.getAttribute('data-product-step')))
            }
          })
        }
        break
        ...
        new Swiper(swiperList[i], {
          ...
          breakpoints: {
            1024: {
              ...
              slidesPerView: 4,
              spaceBetween: 16,
              navigation: { nextEl: '.similars_next', prevEl: '.similars_prev' }
            }
          }
        }) 
        break
      case 'selectbox':
        ...
        break
      case 'certificates':
        const modalbox = getParentByClassname(swiperList[i], 'modalbox')
        const certificatesSteps = modalbox.querySelector('[data-certificates-steps]')
        const setCertificatesSteps = (s) => {
          if(certificatesSteps) certificatesSteps.innerHTML = `${s.realIndex + 1} / ${s.slides.length}`
        }
        window.swipers.certificates = new Swiper(swiperList[i], {
          slidesPerView: 3,
          centeredSlides: true,
          navigation: { nextEl: '.certificates_next', prevEl: '.certificates_prev' },
          on: {
  ...
  const bigLogoAnimation = document.querySelector('[data-animation-big-logo]')
  const bigLogoAnimationTriger = document.querySelector('[data-animation-big-logo-triger]')
  if(bigLogoAnimation) {
    new ScrollMagic.Scene({
  ...
  const smallLogoAnimation = document.querySelector('[data-animation-small-logo]')
  ...
  // Scrollbars
  const scrollbars = Array.from(document.querySelectorAll('[data-scrollbar]'))
  for(let i in scrollbars) {
    new PerfectScrollbar(scrollbars[i], { wheelSpeed:0.5 })
  }
  // Modals
  const modalBtns = Array.from(document.querySelectorAll('[data-modal-triger]'))
  let isModal = false
  ...
  for(let i in modalBtns) {
    modalBtns[i].addEventListener('click', ({ target }) => {
      target = target.classList.contains('modalbox') ? target : getParentByNode(target, 'BUTTON')
      if(target) {
        const triger = target.classList.contains('modalbox') ? target.getAttribute('data-modal-target') : target.getAttribute('data-modal-triger')
        target = document.querySelector(`[data-modal-target="${triger}"]`)
        if(target) toggleModal(target)
      }
    })
  }
  // Accordeons
  const accordionBtns= Array.from(document.querySelectorAll('[data-accordion]'))
  ...
  const optionBtns = Array.from(document.querySelectorAll('[data-select-value]'))
  for(let i in optionBtns) {
    optionBtns[i].addEventListener('click', ({ target }) => {
      ...
      window.swipers.selectbox.slideTo(parseInt(target.getAttribute('data-select-value')))
    })
  }
  // Datepicker
  ...
  // Gallery
  window.lightGalleryInstance = lightGallery(document.querySelector('[lightgallery]'))
  // Test
  const btn = document.querySelector('[data-test]')
  btn && btn.addEventListener('click', ({ target }) => {
  ...
  const setState = (target) => {
    target.classList[validation.isNotEmpty(target) ? 'add' : 'remove']('ready')
  ...
  for(let i in formsValidate) {
    formsValidate[i].addEventListener('submit', event => {
      ...
      const validations = Array.from(formsValidate[i].querySelectorAll('[data-validate]'))
      let err = false
      for(let e in validations) {
        const rule = validations[e].getAttribute('data-validate')
        let check = true
      ...
      if(!err) {
        const openModal = getParentByClassname(formsValidate[i], 'modal')
        openModal && toggleModal(openModal)
        setTimeout(() => {
          const modal = document.querySelector(`[data-modal-target="${testResponse ? 'feedback_success' : 'feedback_error'}"]`)
...
/*** Adaptive ***/
  const subNav = Array.from(document.querySelectorAll('header .sub')),
        mobNav = document.querySelector('header .nav')

  if(header) {
    burgerBtn.addEventListener("click", () => {
      header.classList.toggle('mob-active')
    })

    for(let i in subNav) {
      let subLink = subNav[i].querySelector('.sub-link')
      
      subLink.addEventListener("click", () => {
        subNav[i].classList.toggle('js-show')
        if(subNav[i].classList.contains('js-show')) {
          mobNav.classList.add('js-hide')
        } else {
          mobNav.classList.remove('js-hide')
        }
      })
    }
  }

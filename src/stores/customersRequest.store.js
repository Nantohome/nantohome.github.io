// stores/customersRequest.store.js
import { defineStore } from 'pinia'
import { useLoaderStore, useApiMsgStore } from '@/stores'
import { handleServerError } from '@/utils/helpers'
import {
	getAdminCustomersRequestApi,
	setAdminCustomersVerifyApi
} from '@/api'

export const useCustomersRequestStore = defineStore({
	id: 'customersRequest',
	state: () => ({
		list: {
			data: null,
	...
	actions: {
		async getData() {
			...
			try {
				const res = await getAdminCustomersRequestApi(this.list.meta.limit, this.list.meta.page, this.list.sortBy, this.formattedSortOrder)
		...
		async searchKeyword(str) {
			...
			try {
				this.list.animated = true
				const res = await getAdminCustomersRequestApi(this.list.meta.limit, this.list.meta.page, this.list.sortBy, this.formattedSortOrder, str)
		...
		checkAllSelected() {
			if(this.list.checkAll) {
				...
				this.list.data.forEach((item) => {
					this.list.selected.push(item.customerId)
				})
			} else {
				this.list.selected = []
			}
		...
		async verifySelected(users = this.list.selected) {
			...
			loaderStore.setLoader(true, 'customersRequestVerifyBtn')

			try {
				await setAdminCustomersVerifyApi(users, true)
		...
		async refuseSelected(users = this.list.selected) {
			...
			loaderStore.setLoader(true, 'customersRequestRefuseBtn')

			try {
				await setAdminCustomersVerifyApi(users, false)
				await loaderStore.setLoader(false)
